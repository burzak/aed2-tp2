...
\InterfazFuncion{Pokemons}{\In {j}{Jugador}, \In {g}{juego}}{ItPokemon}
[j $\in$ jugadores(g)]
...
\tadOperacion{deMulticonjAConj}{multiconj(pokemon)}{conj(<pokemon, nat>)}{}

\tadAxioma{deMulticonjAConj(mp)}{\IF $\emptyset$(mp) THEN $\emptyset$ ELSE Ag(<DameUno(mp), $\#$(DameUno(mp), mp)>, deMulticonjAConj(SinUno(mp))) FI}
...
\ttitulo{Operaciones del iterador Jugador}\\

\InterfazFuncion{CrearIt}{\In {g}{Juego}}{itJugador}
{res = CrearItUni(v)}
[O(1)]
...
\ttitulo{Operaciones del iterador Pokemons}

\InterfazFuncion{CrearIt}{\In {g}{Juego}, \In {j}{jugador}}{itPokemon}
[j $\in$ jugadores(g)]
...
InfoJugador representa la informaci\'on de cada jugador. Aqu\'i guardamos al jugador (para no depender solamente del la posici\'on del vector que algunas veces lo recorremos pero no lo tenemos), si esta conectado, expulsado, sanciones, en pos la posici\'on actual, pokemons atrapados, la posici\'on en el mapa, cantTotalPoke es cantidad total de pokemones que atrapo. En pokemons guardamos un iterador  de lista que se guarda en Game, se explic\'o en (\textbf{**}). En posicionMapa guardamos un iterador a la cola de prioridad (heap) de la coordenada en caso de estar esperando para atrapar un pokemon, esto lo hicimos para poder eliminarlo de forma f\'acil del heap (cuando se mueve, se elimina, etc) y poder cumplir las complejidades de mover.
	...
	\begin{Tupla}[InfoJugador]
		...
		\tupItem{\\posicionMapa}{itDicc(jugador: Nat, EsperandoCapturar: itColaDePrioridad(T, menorT)))}
	...
	\begin{Tupla}[InfoCoordenada]
		...
		\tupItem{\\jugEspe}{ColaDePrioridad(T, menorT)}
		\tupItem{\\hayPokemon}{Bool}
		\tupItem{\\yaSeCapturo}{Bool}
		\tupItem{\\jugadores}{Dicc(jugador: nat, EsperandoCapturar: itColaDePrioridad(T, menorT))}
		\tupItem{\\MovimientosRestantes}{Nat}
	\end{Tupla}

	\begin{Tupla}[T]
		\tupItem{pokemon}{Pokemon}
		\tupItem{\\cantidadPokemonesCapturados}{Nat}
	\end{Tupla}
...
\Rep[Game $\quad$ G][G]{(1)(($\forall$ j: Jugador)(0 $\leq$ j $\leq$ Longitud(G.jugadores)) $\impluego$ ($\forall$ p: Pokemons)(Definido?(p, Siguiente(G.jugadores[j].pokemons))) $\Rightarrow$ Definido?(p, G.pokemons) $\yluego$ Significado(p, Siguiente(G.jugadores[j].pokemons)) $\leq$ Significado(p, G.pokemons).cantTotal )$\wedge$ (2) (SumarDicc(claves(G.pokemons), G.pokemons) $=$ G.cantidadTotPokemons) $\wedge$  (3) (($\forall$ i, j :nat)(Definido?(G.mapaInfo, [i, j])) $\Rightarrow$ ($\exists$ c :Coordenada)(c $\in$ Coordenadas(G.mapa)) $\Rightarrow$ i $=$ Latitud(c) $\wedge$ j $=$ Longitud(c)) $\wedge$ (4) ($\#$(G.coordenadaConPokemons) = (pokemonsSalvajes(claves(G.pokemons), G.pokemons))) $\wedge$ (5) (($\forall$ i :Jugador)(0 $\leq$ j $<$ Longitud(G.jugadores)) $\impluego$ PosExistente(G.jugadores[i].pos, G.mapa) $\wedge$ Definido?(G.mapaInfo, G.jugadores[i].pos)) $\wedge$ (6)  (($\forall$ i :Jugador)(0 $\leq$ j $<$ Longitud(G.jugadores)) $\impluego$ i = G.jugadores[i].jug) $\wedge$ (7) ($\forall$ c: Coordenada)(c $\in$ G.coordenadasConPokemons) $\impluego$ ($\forall$ x: G.mapaInfo[c].jugEspe)($\exists$ j:jugadores)(0 $\neq$ j < Longitud(G.jugadores) $\yluego$ distEuclidea(c, G.jugadores[j].pos) < 25 $\wedge$ hayCamino(c, G.jugadores[j].pos)) $\impluego$ x = <G.jugadores[j].cantTotalPoke, j> $\wedge$ (8) (($\forall$ c: Coordenada)(Definido?(c, G.mapaInfo)) $\impluego$ G.mapaInfo[c].hayPokemon $\impluego$ Definido(G.mapaInfo[c].pokemon, G.pokemons) $\wedge$ c $\in$ G.coordenadasConPokemons) $\wedge$ (9) (($\forall$ j: jugador)(0 $\leq$ j $<$ Longitud(G.jugadores) $\yluego$ G.jugadores[j].expulsado = false)  $\impluego$ SumaPokemon(claves(Siguiente(G.jugadores[j].pokemons)), Siguiente(G.jugadores[j].pokemons)) = G.jugadores[j].cantTotalPoke)} 

\tadOperacion{pokemonsSalvajes}{conj(pokemons) cp, dicc(pokemons, <nat, nat> d)}{nat}{($\forall$ p: Pokemon) (p $\in$ cp) $\Rightarrow$ (Definido?(x, d))}

\tadAxioma{pokemonsSalvajes(cp, d)}{\IF $\emptyset$(cp) THEN 0 ELSE $\pi_1$(Significado(DameUno(cp), d)) + pokemonsSalvajes(SinUno(cp), d) FI}

\tadOperacion{SumarDicc}{Conj(string)/cs, Dicc(stirng, $<$nat, nat$>$)/d}{nat}{($\forall$ s: Sting)(s $\in$ cs) $\Rightarrow$ (Definido?(x, d))}

\tadAxioma{SumarDicc(cs, d)}{\IF $\emptyset$(cs) THEN 0 ELSE $\pi_2$(Significado(DameUno(cs), d)) + SumarDicc(SinUno(cs), d) FI}

\tadOperacion{SumarPokemon}{Conj(string)/cs, Dicc(stirng, nat)/d}{nat}{($\forall$ s: Sting)(s $\in$ cs) $\Rightarrow$ (Definido?(x, d))}

\tadAxioma{SumarPokemon(cs, d)}{\IF $\emptyset$(cs) THEN 0 ELSE Significado(DameUno(cs), d) + SumarPokemon(SinUno(cs), d) FI}


\subsubsection{Predicado de abtraccion}

\AbsFc[Game]{juego}[g]{ j :Juego| (g.mapa = mapa(j)) $\wedge$ (g.jugadores = jugadores(j)$\cup$expulsados(j) $\yluego$ ($\forall$ x :Jugador)(x $\in$ j.jugadores) $\impluego$ estaConectado(x, j) = g.jugadores[x].conectado $\wedge$ sanciones(x, j) = g.jugadores[x].sanciones $\wedge$ posicion(x, j) = g.jugadores[x].pos $\wedge$ deDiccAMulti(claves(Siguiente(g.jugadores[x].pokemons)), Siguiente(g.jugadores[x].pokemons)) = pokemons(x, j)) $\wedge$ (g.coordenadasConPokemons = j.posConPokemon $\yluego$ ($\forall$ c: coordenada)(c $in$ j.posConPokemons) $\impluego$ g.mapaInfo[c].pokemon = pokemonEnPos(c, j) $\wedge$ g.mapaInfo[c].MovimientosRestantes = cantMovimientosParaCapturar(c, j))}

\tadOperacion{deDiccAMulti}{conj(pokemon)/cp, dicc(pokemon, cantidad) d}{multiconj(pokemon)}{($\forall$ p: Pokemon) (p $\in$ cp) $\Rightarrow$ (Definido?(x, d))}

\tadAxioma{deDiccAMulti(cp, d)}{\IF $\emptyset$(cp) THEN $\emptyset$ ELSE agregarATodos(Significado(DameUno(cp), d), DameUno(cp)) $\cup$ deDiccAMulti(SinUno(cp), d) FI}

\tadOperacion{agregarATodos}{nat, pokemon}{multiconj(pokemon)}{}

\tadAxioma{agregarATodos(n, p)}{\IF 0?(n) THEN $\emptyset$ ELSE Ag(p, agregarATodos(n-1, p)) FI}

\ttitulo{Representacion del iterador jugador}
...
\AbsFc[iter]{itPokemon}[I]{ it :Iterador Unidireccional| Siguientes(I) = Siguientes(it)}
...
\begin{algorithm}[H]{\textbf{iCantMovimientosParaCaptura}(\In {c}{Conj(coordenada)}, \In {g}{Game}) $\to$ res: Conj(Jugador)}
	\begin{algorithmic}[1]
	
		\State coord $\gets$ c$\to$celdasValidas
		\State itCoord $\gets$ CrearIt(coord)
		\While {HaySiguiente(itCoord)}
			\If {HayCamino(mapa, Siguiente(itCoord)), c}
				\State itDicc $\gets$ mapaInfo[Siguiente(itCoord)$\to$latitud]
				\While {HaySiguiente(itDicc)}
					j $\gets$ SiguienteClave(itDicc)
					\State Agregar(res, j)
					\State Avanzar(itDicc)
				\EndWhile
			\EndIf
			\State Avanzar(itCoord)
		\EndWhile
...
\ttitulo {Algoritmos del iterador jugadores}
...
\ttitulo {Algoritmos del iterador pokemons}

\begin{algorithm}[H]{\textbf{iCrearIt}(\In {g}{game}, \In {j}{jugador}) $\to$ res:iter}
	\begin{algorithmic}[1]
		\State $res \gets$ CrearIt(Siguiente(g.jugadores[j].pokemons)) \Comment O(1)
		\medskip
		\Statex \underline{Complejidad:} O(1)
...
\begin{algorithm}[H]{\textbf{iHayMas?}(\In {it}{itPokemon}) $\to$ res:bool}
	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iActual}(\In {it}{itPokemon}) $\to$ res:<string, nat>}
	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iAvanzar}(\Inout {it}{itPokemon})}
	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iSiguientes}(\In {it}{itPokemon}) $\to$ res:lista(string)}
	\begin{algorithmic}[1]
