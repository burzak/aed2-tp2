...
\begin{algorithm}[H]{\textbf{iCeldasValidas}(\In {g}{Game}, \In {c}{coordenada}) $\to$ res: lista(coordenada)}
	\begin{algorithmic}[1]
		...
		\While {i > 0} \Comment O(1)
			...
			\EndIf			
			\If{latitud(c)-(i-1) >0}			
				\State AgregarAtras(ls, <latitud(c)-(i-1), longitud(c)+1>) \Comment O(1)					\EndIf			
			\State i $\gets$ i -1 \Comment O(1) 
		\EndWhile
...
\begin{algorithm}[H]{\textbf{iMoverse}(\In {j}{jugador}, \In {c}{coordenada}, \Inout {g}{Game})}
	\begin{algorithmic}[1]
		\State ActualizarJugador(j, c, g) \Comment O(log $\#$(elementosEnColaDeC) + log $\#$(elementosColaDeCordenadaDelJugador) + $\#$(pokemonsDiccDeJugador)*|P|)
...
\begin{algorithm}[H]{\textbf{iActualizarJugadorYCoordenadas}(\In {j}{jugador}, \In {c}{coordenada}, \Inout {g}{Game})}
	\begin{algorithmic}[1]
		\If {SiguienteSignificado(g.jugadores[j].posicionMapa) $\neq$ NULL} \Comment O(log $\#$(elementosColaDeCordenadaDelJugador))
			\State Borrar(SiguienteSignificado(g.jugadores[j].posicionMapa)) \Comment O(log $\#$(elementosColaDeCordenadaDelJugador))
			\State EleminarSiguiente(g.jugadores[j].posicionMapa) \Comment O(1)							\State g.jugadores[j].posicionMapa $=$ NULL \Comment O(1)
		\EndIf		
		...
		\If {g.jugadores[j].expulsado $=$ true} \Comment O($\#$(pokemonsDicc)*|P|)
			...
			\State itDicc(pokemon, nat) itPokemons $\gets$ CrearIt(Siguiente(g.jugadores[j].pokemons)) \Comment O(1)
			\While{HaySiguiente(itPokemons)} \Comment O($\#$(pokemonsDicc)*|P|)
				...
				\If {Significado(g.pokemones, SiguienteClave(it.Pokemons)).cantTotal = 0} \Comment O(|P|)
		...
		\EndIf
		\medskip
		\Statex \underline{Complejidad:} O(log $\#$(elementosEnColaDeC) + log $\#$(elementosColaDeCordenadaDelJugador) + $\#$(pokemonsDicc)*|P|), donde P es la máxima longitud de las claves de diccionario
		\Statex \underline{Justificacion:}Primero Borro lo que apunta el iterador de cola de prioridad, esto me cuesta O(log $\#$(elementos de la Cola que apunta J)) y borro el it a diccionario y le asigno null (podria quedar apuntando a otro elemento de la lista). Luego chequeo si el jugador cometió una infracción, si lo hizo le sumo una sanción. Entonces veo si lo tengo que expulsar, si lo hago resto su cantidad de pokemons al total de pokemons en el juego porque desaparecen, despues por cada pokemon del diccionario del jugador, resto su cantidad a su respectivo pokemon en el diccString (resto a su cantidadTotal en el diccString), esto me toma O($\#$(elementos del dicc) * |P|). Si no lo tengo que expulsar simplemente defino su nueva coordenada y veo si tengo que agregarlo a una nueva cola de prioridad o no, si tengo que hacerlo tengo que revalanciar la cola eso me cuesta O(log $\#$(elementos de la cola en la coordenada c)), sino es constante. Entonces la complejidad final es O(log $\#$(elementosEnColaDeC) + log $\#$(elementosColaDeCordenadaDelJugador) + $\#$(pokemonsDicc)*|P|). Para chequear que la complejidad cumple la pedida (La función la uso en moverse) veo que $\#$(elementos del diccionario de pokemons del jugador) es a lo sumo PC (máxima cantidad de pokemons atrapados por un jugador), cuando lo multiplico por |P|, veo que la complejidad está contenida en O((PS + PC) * |P|). Para la otra parte de la complejidad veo que log $\#$(elementos de la cola de C) y log $\#$(elementos de la cola de J) se acotan por log (EC) (donde EC es la máxima cantidad de jugadores esperando capturar un pokemon) esto pasa porque ambas colas están contempladas en la complejidad, pasaría, en peor caso, O(2*log(EC)) que quedaría O(log(EC)), entonces la complejidad de esta función está contenida en la pedida.
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iVerCapturas}(\In {j}{jugadores}, \In {c}{coordenadas}, \Inout {g}{Game})}
	\begin{algorithmic}[1]
		...
		\While{HaySiguiente(itPokeCoordenadas)} \Comment O($\#$(elementosEnConj) * |PenJ| + log $\#$(elementosCola)))
			\If{Siguiente(itPokeCoordenadas) $neq$ coordeJ} \Comment O(1)
				...
				\If{g.mapaInfo[Siguiente(itPokeCoordenadas)].MovimientosRestantes $\geq$ 10} \Comment O(1) 
					...
					\If{Definido?(Siguiente(g.jugadores[jug].pokemons), g.mapaInfo[Siguiente(itPokeCoordenadas)].pokemon)} \Comment O(|PenJ|))									\State Significado(Siguiente(g.jugadores[jug].pokemons), g.mapaInfo[Siguiente(itPokeCoordenadas)].pokemon) $\gets$ Significado(Siguiente(g.jugadores[jug].pokemons), g.mapaInfo[Siguiente(itPokeCoordenadas)].pokemon)	+1 \Comment O(|PenJ|)								\Else
						\State DefinirRapido(Siguiente(g.jugadores[jug].pokemons), g.mapaInfo[Siguiente(itPokeCoordenadas)].pokemon, 1) \Comment O(1)
					\EndIf 			
					\State Borrar(SiguienteSignificado(g.jugadores[jug].posicionMapa)) \Comment O(log $\#$(elementosCola))					
				\EndIf
		...
		\EndWhile
		\medskip
		\Statex \underline{Complejidad:}O($\#$(elementosEnConj) * |PenJ| + log $\#$(elementosCola)))
		\Statex \underline{Justificacion:}
	\end{algorithmic}
\end{algorithm}
