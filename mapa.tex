...
\textbf{Formal}\\
\Rep[infoMapa $\quad$ M][M]{(1)($\exists$ $c_1$ :Coordenada)($c_1$ $\in$ M.coordenadas) $\impluego$ latitud($c_1$) = M.alto $\wedge$ ($\exists$ $c_2$ :Coordenada)($c_2$ $\in$ M.coordenadas) $\impluego$ longitud($c_2$) = M.ancho $\wedge$ (2) }
