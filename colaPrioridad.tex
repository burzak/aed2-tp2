...
\textbf{Formal}
\Rep[colaMin C][C]{(2) C.raiz->padre = NULL $\wedge$ (3) ($\forall$ p: nodoHeap($\sigma$)) p.der $\neq$ NULL $\Rightarrow$ p.izq $\neq$ NULL $\wedge$ (4) ($\forall$ p: nodoHeap($\sigma$)) p.clave $>$ p.izq->clave $\wedge$ p.clave $>$ p.der->clave }


\subsection{Predicado de abstracción}

\AbsFc[colaM]{colaMin}[C]{ cola :Cola($\sigma$)| EsVacia?(C) = vacia(cola) $\yluego$ (Proximo(C) = proximo(cola) $\wedge$ Desencolar(C) = desencolar(cola))}
...
No es posible hacerlo porque el iterador no puede avanzar, el iterador solo esta hecho para ser usado como un puntero con la ventaja de no revelar la estructura interna. Su finalidad es permitir un borrado rapido ya que no se tiene que buscar el elemento a borrar.

\subsection{Algoritmos}

\ttitulo{Funciones Auxiliares, no exportadoas}
