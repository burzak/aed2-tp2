...
void Juego::Conectarse(Jugador j, Coordenada c){							//Complejidad: O(log(EC))
	...
	if(HayPokemonCercano(c) && mapa.HayCamino(PosPokemonCercano(c), c)){				//O(log(EC))
...
void Juego::Moverse(Jugador j, Coordenada c){								//Complejidad: O(log(EC) + |P| * (PC + PS))
	VerCapturas(j, c);										//O(|P| * PS)
	ActualizarJugadorYcoordenada(j, c);								//O(log(EC) + |P| * PC)
};
...
Conj<Jugador> Juego::JugadoresConectados(){
	Vector<InfoJugador>::const_Iterador it = jugadores.CrearIt();					//O(J)
	Conj<Jugador> c;
	while(it.HaySiguiente()){
...
bool Juego::HayPokemonCercano(const Coordenada c) {							//O(1)
	bool res = false;
	Lista<Coordenada> coordCercanas = CeldasValidas(c);						//Son maximo 13 coordenadas
	Lista<Coordenada>::const_Iterador iter = coordCercanas.CrearIt();

	while (iter.HaySiguiente()) {									//O(13) = O(1)
...
Nat Juego::IndiceRareza(Pokemon p){									//O(|P|), accedes una unica vez al diccionario de pokemones
	Nat cant = pokemons.Significado(p).second;
	return 100-((cant*100)/cantidadTotPokemons);
};

Nat Juego::CantPokemonsTotales(){									//O(1)
	return cantidadTotPokemons;
};

Coordenada Juego::PosPokemonCercano(Coordenada c){							//O(1)
	Coordenada res(0 ,0);
	Lista<Coordenada> coordCercanas = CeldasValidas(c);
	Lista<Coordenada>::const_Iterador iter = coordCercanas.CrearIt();
	while (iter.HaySiguiente()){									//Se hace el chequeo sobre 13 posiciones O(13) = O(1)
		Coordenada coordAux = iter.Siguiente();
...
void Juego::ActualizarJugadorYcoordenada(Jugador j, Coordenada c){						//Complejidad: O(log(EC) + PC * |P|)
	...
	if(jugadores[j].expulsado == true){
		...
		Lista<string>::const_Iterador itPokemons = jugadores[j].pokemons.Siguiente().Claves().CrearIt();//O(1)
		while(itPokemons.HaySiguiente()) {								//O(PC * |P|)
			pokemons.Significado(itPokemons.Siguiente()).second -= jugadores[j].pokemons.Siguiente().Significado(itPokemons.Siguiente());
			itPokemons.Avanzar();
		}
		jugadores[j].pokemons.EliminarSiguiente();							//O(1)
		jugadores[j].cantTotalPoke = 0;
		if (jugadores[j].posicionMapa.HaySiguiente() && jugadores[j].posicionMapa.SiguienteSignificado().HayMas()) {
			jugadores[j].posicionMapa.SiguienteSignificado().Borrar();				//O(log(EC))
		}
		if (jugadores[j].posicionMapa.HaySiguiente()) {							//O(1)
		...
		if (distanciaEuclidea(c, jugadores[j].pos) <= 100 && mapa.HayCamino(c, jugadores[j].pos)) {
			...
			if (!HayPokemonCercano(jugadores[j].pos) && !HayPokemonCercano(c)) {			//O(1)
				// no se hace nada en este caso
			} else if (HayPokemonCercano(jugadores[j].pos) && !HayPokemonCercano(c)) {		//O(1)
				...
				mapaInfo[pokeCoor.latitud][pokeCoor.longitud].movimientosRestantes = 10;	
			} else if (PosPokemonCercano(c) != PosPokemonCercano(jugadores[j].pos)) {		//O(1)
	...
	if (distanciaEuclidea(c, jugadores[j].pos) <= 100 && mapa.HayCamino(c, jugadores[j].pos)) {		//O()
		jugadores[j].pos = c;

		if (jugadores[j].posicionMapa.HaySiguiente() && jugadores[j].posicionMapa.SiguienteSignificado().HayMas()) {	//log(EC)
...
void Juego::VerCapturas(Jugador j, Coordenada c){								//Complejidad O(|P| * PS)
	// si es sancionado no verificamos nada de capturas
	if(distanciaEuclidea(c, jugadores[j].pos) > 100 || !(mapa.HayCamino(c, jugadores[j].pos))){		//O(1)
		return;
	}

	Conj<Coordenada>::Iterador iter = coordenadasConPokemons.CrearIt();

	while (iter.HaySiguiente()){										//O(|P| * PS)
		if (distanciaEuclidea(iter.Siguiente(), c) > 25) {						//O(1)
			Infocoordenada* infocoor = &mapaInfo[iter.Siguiente().latitud][iter.Siguiente().longitud];//O(1)
			...
			if (infocoor->movimientosRestantes == 0){
				...
				if (!infocoor->jugEspe.preguntarVacia()){
					...
					if (jugadores[jug].pokemons.Siguiente().Definido(infocoor->pokemon)){	//O(|P|)
...
const Mapa& Juego::MapaDeJuego() const {									//O(1)
	return mapa;
}
